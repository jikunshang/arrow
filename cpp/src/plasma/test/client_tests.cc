...
class TestPlasmaStore : public ::testing::Test {
 public:
  ...
  void SetUp() {
    ASSERT_OK(TemporaryDir::Make("cli-test-", &temp_dir_));
...
void AssertCudaRead(const std::shared_ptr<Buffer>& buffer,
  ...
  ASSERT_OK(reader.Read(data_size, &read_data_size, read_data.data()));
...
TEST_F(TestPlasmaStore, GetGPUTest) {
  ...
  ASSERT_OK(CudaBuffer::FromBuffer(data_buffer, &gpu_buffer));
...
TEST_F(TestPlasmaStore, RepeatlyCreateGPUTest) {
  ...
  for (int64_t i = 0; i < object_num; i++) {
    ...
    std::shared_ptr<Buffer> data;
    ASSERT_OK(client_.Create(object_id, data_size, 0, 0, &data, 1));
    ASSERT_OK(client_.Seal(object_id));
    ASSERT_OK(client_.Release(object_id));
  }
  ...
  // delete all
  ASSERT_OK(client_.Delete(object_ids));
}
